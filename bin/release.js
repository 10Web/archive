 ...
 * @param {String} dir
 */
exports.front = (config, dir, {run, log, fs, co}) => {
  ...
  return co(function* () {
    ...
    if (needToInstall) {
      run(`cd ${assetsDir} && cnpm install --${config.env}`, 'install frontend dependences');
      run(`cd ${assetsDir} && cnpm install honeypack`, 'install honeypack');
 ...
 * @param {*} dir
 */
exports.release = (config, dir, {run, log, fs, co}) => {
  ...
  return co(function* () {
    ...
    // 安装依赖
    run(`cd ${releaseDir} && cnpm install --${config.env}`, 'install project dependences');
